...
void GRPCClient::SendComplete() {
  ...
  if (!completed_) {
    for (auto& it : channels_) {
      SPDLOG_INFO("send complete message to {}", it.first);
...
RpcHandlePtr GRPCClient::CreateAsync(const std::string& ep,
  ...
  VoidProcessor* c =
      new VoidProcessor(ch, "/klearn.KlearnService/Create", req,
  ...
  pool_.Run(
      [ch, h, batch_id, varname, &options, val1, val2, timeout_in_ms, this] {
        ...
        EncodeTensorToByteBuffer(ROLE_HUB, role_id_, batch_id, varname, options,
                                 val1, val2, &req);
        VoidProcessor* c =
            new VoidProcessor(ch, "/klearn.KlearnService/FeedSample", req,
                              FLAGS_rpc_retry_times, h, timeout_in_ms, GetCq());
        c->Prepare();
        c->StartCall();
      });
...
RpcHandlePtr GRPCClient::PullAsync(const std::string& ep, uint64_t batch_id,
  ...
  pool_.Run([ch, response, h, req, timeout_in_ms, this] {
    MsgProcessor* c = new MsgProcessor(ch, "/klearn.KlearnService/Pull", req,
...
RpcHandlePtr GRPCClient::PushAsync(const std::string& ep, uint64_t batch_id,
  ...
  pool_.Run([ch, h, req, timeout_in_ms, this] {
    VoidProcessor* c =
        new VoidProcessor(ch, "/klearn.KlearnService/Push", req,
                          FLAGS_rpc_retry_times, h, timeout_in_ms, GetCq());
    c->Prepare();
    c->StartCall();
  });
...
RpcHandlePtr GRPCClient::EmbeddingLookupAsync(
  ...
  pool_.Run([ch, response, h, req, timeout_in_ms, this] {
    MsgProcessor* c = new MsgProcessor(
        ch, "/klearn.KlearnService/EmbeddingLookup", req, FLAGS_rpc_retry_times,
...
RpcHandlePtr GRPCClient::PushGradAsync(
  ...
  pool_.Run(
      [ch, h, batch_id, varname, &options, val1, val2, timeout_in_ms, this] {
        ...
        VoidProcessor* c =
            new VoidProcessor(ch, "/klearn.KlearnService/PushGrad", req,
                              FLAGS_rpc_retry_times, h, timeout_in_ms, GetCq());
        c->Prepare();
        c->StartCall();
      });
...
RpcHandlePtr GRPCClient::SaveAsync(const std::string& ep,
  ...
  const auto ch = GetChannel(ep);
  VoidProcessor* c =
      new VoidProcessor(ch, "/klearn.KlearnService/Save", req,
                        FLAGS_rpc_retry_times, h, timeout_in_ms, GetCq());
  c->Prepare();
  c->StartCall();
  return h;
...
RpcHandlePtr GRPCClient::RestoreAsync(
  ...
  MsgProcessor* c = new MsgProcessor(ch, "/klearn.KlearnService/Restore", req,
                                     FLAGS_rpc_retry_times, h, timeout_in_ms,
                                     GetCq(), response);
  c->response_call_back_ = ProcMsgResponse;
  c->Prepare();
...
RpcHandlePtr GRPCClient::FreezeAsync(const std::string& ep,
  ...
  const auto ch = GetChannel(ep);
  VoidProcessor* c =
      new VoidProcessor(ch, "/klearn.KlearnService/Freeze", req,
                        FLAGS_rpc_retry_times, h, timeout_in_ms, GetCq());
  c->Prepare();
  c->StartCall();
  return h;
...
RpcHandlePtr GRPCClient::CompleteAsync(const std::string& ep,
  ...
  const auto ch = GetChannel(ep);
  VoidProcessor* c =
      new VoidProcessor(ch, "/klearn.KlearnService/Complete", req,
                        FLAGS_rpc_retry_times, h, timeout_in_ms, GetCq());
  c->Prepare();
  c->StartCall();
  return h;
...
RpcHandlePtr GRPCClient::StartSampleAsync(
  ...
  const auto ch = GetChannel(ep);
  VoidProcessor* c =
      new VoidProcessor(ch, "/klearn.KlearnService/StartSample", req,
                        FLAGS_rpc_retry_times, h, timeout_in_ms, GetCq());
  c->Prepare();
  c->StartCall();
  return h;
...
RpcHandlePtr GRPCClient::ReadSampleAsync(const std::string& ep,
  ...
  pool_.Run([ch, response, h, req, timeout_in_ms, this] {
    MsgProcessor* c = new MsgProcessor(ch, "/klearn.KlearnService/ReadSample",
...
RpcHandlePtr GRPCClient::UpdateHubShardAsync(
  ...
  const auto ch = GetChannel(ep);
  VoidProcessor* c =
      new VoidProcessor(ch, "/klearn.KlearnService/UpdateHubShard", req,
                        FLAGS_rpc_retry_times, h, timeout_in_ms, GetCq());
  c->Prepare();
  c->StartCall();
  return h;
...
RpcHandlePtr GRPCClient::HeartbeatAsync(
  ...
  const auto ch = GetChannel(ep);
  MsgProcessor* c = new MsgProcessor(ch, "/klearn.KlearnService/Heartbeat", req,
                                     FLAGS_rpc_retry_times, h, timeout_in_ms,
                                     GetCq(), response);
  c->response_call_back_ = ProcMsgResponse;
  c->Prepare();
...
RpcHandlePtr GRPCClient::CountFeatureAsync(
  ...
  const auto ch = GetChannel(ep);
  MsgProcessor* c = new MsgProcessor(ch, "/klearn.KlearnService/CountFeature", req,
                                     FLAGS_rpc_retry_times, h, timeout_in_ms,
                                     GetCq(), response);
  c->response_call_back_ = ProcMsgResponse;
  c->Prepare();
...
RpcHandlePtr GRPCClient::SaveFeatureCountAsync(const std::string& ep,
  ...
  const auto ch = GetChannel(ep);
  MsgProcessor* c = new MsgProcessor(ch, "/klearn.KlearnService/SaveFeatureCount", req,
                                     FLAGS_rpc_retry_times, h, timeout_in_ms,
                                     GetCq(), response);
  c->response_call_back_ = ProcMsgResponse;
  c->Prepare();
...
RpcHandlePtr GRPCClient::RestoreFeatureCountAsync(const std::string& ep,
  ...
  const auto ch = GetChannel(ep);
  MsgProcessor* c = new MsgProcessor(ch, "/klearn.KlearnService/RestoreFeatureCount", req,
                                     FLAGS_rpc_retry_times, h, timeout_in_ms,
                                     GetCq(), response);
  c->response_call_back_ = ProcMsgResponse;
  c->Prepare();
...
void GRPCClient::Proceed(grpc::CompletionQueue* cq) {
  void* tag = nullptr;
  bool ok = false;
  SPDLOG_DEBUG("GRPCClient Proceed begin");
...
std::shared_ptr<grpc::Channel> GRPCClient::GetChannel(const std::string& ep) {
  ...
  SPDLOG_DEBUG("Setting GRPC compression : algo= {0},  level={1}",
               FLAGS_grpc_client_compression_algo,
