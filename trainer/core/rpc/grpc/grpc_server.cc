...
void AsyncGRPCServer::WaitServerReady() {
  SPDLOG_INFO("AsyncGRPCServer is waiting server ready");
  std::unique_lock<std::mutex> lock(this->mutex_ready_);
  condition_ready_.wait(lock, [=] { return this->ready_ == 1; });
  SPDLOG_INFO("AsyncGRPCServer WaitSeverReady");
}
...
void AsyncGRPCServer::StartServer() {
  ...
  if (FLAGS_rpc_server_disable_reuse_port) {
    ...
    SPDLOG_INFO("set FLAGS_rpc_server_disable_reuse_port");
  }
  ...
  if (server_ != nullptr) {
    SPDLOG_INFO("Server listening on {} successful", bind_address_);
    service_.reset(service.release());
  } else {
    SPDLOG_CRITICAL("Server listening on {} successful", bind_address_);
    abort();
  }
  ...
  for (auto& t : rpc_call_map_) {
    ...
    for (int i = 0; i < kRequestBufSize; i++) {
      SPDLOG_DEBUG("TryToRegisterNewOne on RPC NAME: {0}{1}", rpc_name, i);
    ...
    for (int i = 0; i < threadnum; i++) {
      ...
      SPDLOG_DEBUG("{} creates threads!", t.first);
    }
  }
  ...
  for (auto& t : rpc_threads_) {
    ...
    for (size_t i = 0; i < threads.size(); ++i) {
      threads[i]->join();
      SPDLOG_INFO("{} threads ends!", t.first);
    }
  }
}
...
void AsyncGRPCServer::ShutdownQueue() {
  for (auto& t : rpc_cq_) {
    t.second->Shutdown();
    SPDLOG_INFO("{} queue shutdown!", t.first);
  }
}
...
void AsyncGRPCServer::TryToRegisterNewOne(const std::string& rpc_name,
  ...
  std::unique_lock<std::mutex> lock(cq_mutex_);
  if (is_shut_down_) {
    SPDLOG_INFO("shutdown, do not TryToRegisterNewSendOne");
    return;
  }

  SPDLOG_DEBUG("TryToRegisterNewOne on RPC NAME: {0} REQ ID: {1}", rpc_name,
               req_id);
  ...
  SPDLOG_DEBUG("TryToRegisterNewOne status: {}", b->Status());
}
...
void AsyncGRPCServer::HandleRequest(
  ...
  while (true) {
    SPDLOG_DEBUG("HandleRequest {} wait next", rpc_name);
    if (!cq->Next(&tag, &ok)) {
      SPDLOG_DEBUG("CompletionQueue {} shutdown!", rpc_name);
      break;
    }

    int req_id = static_cast<int>(reinterpret_cast<intptr_t>(tag));
    SPDLOG_DEBUG("HandleRequest {0} , req_id: {1} get next", rpc_name, req_id);
    ...
    SPDLOG_DEBUG("{}", base->Status2String(rpc_name));
    ...
    if (!ok) {
      SPDLOG_DEBUG("completion queue: {0} recv no regular event context: {1}",
                   rpc_name, base->Status2String(rpc_name));
