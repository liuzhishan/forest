...
#include "core/proto/klearn_service.grpc.pb.h"
...
class BaseProcessor {
 public:
  ...
  void OnCompleted(bool ok) {
    SPDLOG_DEBUG("Completed call: {}", method_);
    ...
    SPDLOG_DEBUG(
        "{0} returned with non-ok status: {1} Retries: {2}  Max: {3} {4}",
        method_, status_.error_code(), num_retries_, max_retries_,
        context_->debug_error_string());

    // Retry if we have any attempts left
    if (++num_retries_ <= max_retries_ &&
        (status_.error_code() == grpc::StatusCode::UNAVAILABLE)) {
      SPDLOG_DEBUG("Retrying call for {0} Retry: {1} of {2}", method_,
                   num_retries_, max_retries_);
      Prepare();
      StartCall();
    } else {
      SPDLOG_DEBUG(
          "{0} meets grpc error, error_code: {1} error_message: {2} "
          "error_details: {3}",
          method_, status_.error_code(), status_.error_message(),
...
class GRPCClient : public RPCClient {
 ...
 private:
  ...
  KLEARN_NOT_COPYABLE_AND_MOVABLE(GRPCClient)
