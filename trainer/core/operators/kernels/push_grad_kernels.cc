...
class PushGradOp : public OpKernel {
 public:
  explicit PushGradOp(OpKernelConstruction* context)
    ...
    SPDLOG_INFO("Trainer{0} push gard op init. sparse_fcs_size={1}",
                trainer_id_, train_config_->sparse_fcs().size());
  }
  ~PushGradOp() {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    SPDLOG_DEBUG("push grad check: batch_id={0}, grad={1}", id,
                 msg->gradient.DebugString());
    {
      if (train_config_->debug_offline()) {
  ...
  void PushGradThread() {
    ...
    while (!exit_flag_) {
      ...
      if (update_time != auto_shard.update_time()) {
        ...
        SPDLOG_INFO("update push grad ps vars, update_time: {}", update_time);
      }
    ...
    if (grad_list_.empty()) {
      SPDLOG_INFO("Trainer{} PushGradThread exit", trainer_id_);
    } else {
      SPDLOG_WARN("Trainer{0} PushGradThread exit, but queue is not empty({1}}",
                  trainer_id_, grad_list_.size());
    }
