 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if ((VaraibleType)vartype_ == VAR_DENSE) {
      ...
      SPDLOG_INFO("pull variable[{0}] success", varname_);
      ...
      for (size_t shard = 0; shard < shard_num; ++shard) {
        ...
        while (!is_completed) {
          ...
          for (auto i = 0; i < key_flat.size(); ++i) {
            auto key = key_flat(i);
            if (key >= capacity) {
              SPDLOG_WARN("pull variable[{0}] fail, key[{1}] >= capacity[{2}]",
                          varname_, key, capacity);
            }
      ...
      SPDLOG_INFO("pull variable[{0}] total_key[{1}] success", varname_,
                  total_key_count);
    } else {
      OP_REQUIRES(ctx, false,
