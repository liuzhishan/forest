...
FeedQueue::FeedQueue(const std::string& conf_file, int32_t trainer_id, int32_t work_mode)
  ...
  init();
  SPDLOG_INFO(
      "Trainer{0} feed queue created. hub_eps_size={1}, parallel={2}, "
      "queue_size={3}, work_mode={4}",
      trainer_id_, hub_eps_.size(), parallel_, queue_size_, work_mode_);
}
    ...
    while (true) {
      {
        ...
        if (queue_.size() < 5) {
          SPDLOG_WARN(
              "Trainer{0} feed queue size({1}) is not rich. training will "
              "bound at prefetch, you can check trainer network and hub / ps "
              "resource.",
              trainer_id_, queue_.size());
        } else {
          SPDLOG_INFO("Trainer{0} feed queue size({1})", trainer_id_, queue_.size());
        }
      }
...
bool FeedQueue::feed(FeatureColumn*& feature, bool* over) {
  ...
  if (!queue_.empty()) {
    ...
    SPDLOG_ERROR("Trainer{} feature is null", trainer_id_);
    return false;
  } else if (is_over()) {
    *over = true;
    return true;
  ...
  while (true) {
    {
      ...
      if (is_over()) {
        SPDLOG_INFO("Trainer{} worker is over", trainer_id_);
        break;
      }
    }
    ...
    if (use_auto_shard_) {
      ...
      if (!auto_shard.is_finish() && auto_shard.is_ready()) {
        if (thread_id == 0) {
          ...
          SPDLOG_INFO("update ps_shard one time, update_time: {}", auto_shard.update_time());
          lk_shard.unlock();
    ...
    if (feature->label_tensor.dim_size(0) != label_size || feature->label_tensor.dim_size(1) != batch_size) {
      SPDLOG_WARN(
          "Trainer{0} label tensor shape[{1}] not match "
          "config[{2},{3}]",
          trainer_id_, feature->label_tensor.shape().DebugString(), batch_size, label_size);
      continue;
    }
    if (feature->dense_tensor.dim_size(0) != batch_size ||
        feature->dense_tensor.dim_size(1) != dense_total_size) {
      SPDLOG_WARN(
          "Trainer{0} dense tensor shape[{1}] not match "
          "config[{2},{3}]",
          trainer_id_, feature->dense_tensor.shape().DebugString(), batch_size, dense_total_size);
      continue;
    }
    ...
    // EmbedddingLookup
    {
      ...
      for (auto& it : ps_var_names) {
        ...
        option.set_work_mode((klearn::WorkMode)work_mode_);
      ...
      for (auto& h : hdls) {
        if (!h->Wait()) {
          SPDLOG_WARN(
              "Trainer{0} embedding lookup fail at batch_id[{1}]. ep={2}, "
              "errmsg={3}",
              trainer_id_, batch_id, h->ep(), h->errmsg());
          lookup_ok = false;
          break;
        }
      }
      ...
      for (auto& it : responses) {
        ...
        if (!option.errmsg().empty()) {
          SPDLOG_WARN(
              "Trainer{0} embedding lookup fail at batch_id[{1}]. ep={2}, "
              "errmsg={3}",
              trainer_id_, batch_id, ep, option.errmsg());
          lookup_ok = false;
          break;
        }
        ...
        if (lookup_ret.dim_size(0) != batch_size || lookup_ret.dim_size(1) != var_total_size) {
          SPDLOG_WARN(
              "Trainer{0} lookup ret tensor shape[{1}] not match "
              "config[{2},{3}]",
              trainer_id_, lookup_ret.shape().DebugString(), batch_size, var_total_size);
          lookup_ok = false;
          break;
        }
        ...
        if (float_lookup_ret.dim_size(0) != batch_size || float_lookup_ret.dim_size(1) != var_total_size) {
          SPDLOG_WARN(
              "Trainer{0} float lookup ret tensor shape[{1}] not match "
              "config[{2},{3}]",
              trainer_id_, float_lookup_ret.shape().DebugString(), batch_size, var_total_size);
          lookup_ok = false;
          break;
        }
        if (!lookup_ok) {
        ...
        for (size_t i = 0; i < var_names.size(); ++i) {
          ...
          if (shard_num == 1) {
            for (int j = 0; j < batch_size; ++j) {
              ...
              SPDLOG_DEBUG(
                  "batch_id={0} var embedding. ep={1}, batch_size={2}, "
                  "var_total_size={3}, i={4}, idx={5}, size={6}, j={7}, "
                  "var_lookup_flat={8}, float_lookup_flat={9}",
                  batch_id, ep, batch_size, var_total_size, i, idx, size, j, var_lookup_flat.size(),
                  float_lookup_flat.size());
              // if (name == "embedding_27") {
              //   LOG(INFO) << "batch_id: " << batch_id
              //             << ", var_name: " << name
              //             << ", j: " << j
              //             << ", debug_info: " << feature->debug_info.flat<std::string>()(j)
              //             << ", shard_num: " << shard_num
              //             << ", ep: " << ep
              //             << ", value: " << JoinFloat(float_lookup_flat.data() + j * var_total_size + idx,
              //             size);
              // }
            }
          } else {
            for (int j = 0; j < batch_size; ++j) {
              Sum(float_lookup_flat.data() + j * var_total_size + idx, var_lookup_flat.data() + j * size,
                  size);
              SPDLOG_DEBUG(
                  "batch_id={0} var embedding. ep={1}, batch_size={2}, "
                  "var_total_size={3}, i={4}, idx={5}, size={6}, j={7}, "
    ...
    for (size_t i = 0; i < input_sparse_dim.size(); ++i) {
      if (feature->embedding_tensor[i].dim_size(0) != batch_size ||
        ...
        SPDLOG_WARN(
            "Trainer{0} feature[{1}] lookup ret shape[{2}] not match "
            "config[{3},{4}]",
            trainer_id_, input_sparse[i], feature->embedding_tensor[i].shape().DebugString(), batch_size,
            input_sparse_dim[i]);
        continue;
      }
    }
    ...
    SPDLOG_DEBUG(
        "feed queue new batch: batch_id={0}, labels={1}, dense={2}, "
        "emb_first={3}, emb_last={4}",
        batch_id, feature->label_tensor.DebugString(), feature->dense_tensor.DebugString(),
        feature->embedding_tensor.front().DebugString(), feature->embedding_tensor.back().DebugString());
...
int32_t FeedQueue::get_next_hub() {
  ...
  SPDLOG_WARN("Trainer{0} get next hub fail", trainer_id_);
  return -1;
}

void FeedQueue::set_hub_over(int32_t idx) {
  std::unique_lock<std::mutex> lk(mu_);
  SPDLOG_WARN("hub[{}] is over!!!", idx);
  hub_over_[idx] = true;
}
...
bool FeedQueue::update_shard() {
  ...
  for (size_t field = 0; field < new_alloc_shard.size(); field++) {
    ...
    if (field >= origin_ps_shard.size()) {
      SPDLOG_INFO("out of range, field: {}, origin_ps_shard.size(): {}", field, origin_ps_shard.size());
      continue;
    }
