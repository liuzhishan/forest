...
class RestoreOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    while (!restore_finish && !restore_error && (--total_iteration > 0)) {
      ...
      if (!h->Wait()) {
        ...
        if (!ret_option.errmsg().empty()) {
          SPDLOG_WARN("Trainer{0} restore fail. ep={1}, errmsg={2}",
    ...
    SPDLOG_INFO(
        "Trainer{0} restore var[{1}] shard_idx[{2}] success on ps[{3}] cost[{4}].",
        trainer_id_, varname_, shard_idx_, ep,
...
REGISTER_KERNEL_BUILDER(Name("KlearnRestore").Device(DEVICE_CPU), RestoreOp);
