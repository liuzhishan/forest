...
void AutoShard::add_lookup_info(size_t ps_index, size_t field, uint64_t time_spend) {
  if (ps_index < lookup_infos_.size()) {
    ...
    SPDLOG_INFO("out of range, ps_index: {}, lookup_infos_.size(): {}", ps_index, lookup_infos_.size());
  }
}
  ...
  if (it_ps_index != ps_to_index.end()) {
    ...
    if (it_varnames != ps_varnames.end()) {
      ...
      if (varnames.size() == static_cast<size_t>(option.time_spends_size())) {
        ...
        SPDLOG_INFO("varnames.size() != option.time_spends_size(), varnames.size(): {}, option.time_spends_size(): {}",
                    varnames.size(), option.time_spends_size());
      }
    } else {
      SPDLOG_INFO("cannot find varnames, ps_name: {}", ps_name);
    }
  } else {
    SPDLOG_INFO("cannot find ps_index, ps_name: {}", ps_name);
  }
}

bool AutoShard::is_ready() {
  if (ps_request_count_ > 0 && ps_request_count_ >= (step_limit_ * ps_count_)) {
    SPDLOG_INFO("auto shard is_ready, ps_request_count_: {}, ps_count_: {}, update_time_: {}",
                ps_request_count_, ps_count_, update_time_);
    return true;
  }
...
std::vector<std::vector<size_t>> AutoShard::compute_shard() {
  ...
  for (size_t ps_index = 0; ps_index < ps_count_; ps_index++) {
    ...
    for (size_t j = 0; j < lookup_infos_[ps_index].size(); j++) {
      if (ps_index < ps_field_load.size()) {
        ...
        if (field < ps_field_load[ps_index].size()) {
          ...
          SPDLOG_INFO("out of range, field: {}, ps_index: {}, ps_field_load[ps_index].size(): {}",
                      field, ps_field_load[ps_index].size(),  ps_field_load[ps_index].size());
        }
      }
    }
    ...
    SPDLOG_INFO("ps_index: {}, ps_load: {}, total_each_field: {}, ps_field_load: {}",
                ps_index,
                ps_load[ps_index],
                absl::StrJoin(total_each_field, ","),
    ...
    for (size_t j = 0; j < top_field_; j++) {
      if (ps_count_ - 1 - i < ps_load_pair.size()) {
        ...
        if (target_ps) {
          ...
          SPDLOG_INFO("add ps, target_field: {}, ps: {}", target_field, *target_ps);
        }
      }
    }
  }
...
void AutoShard::update_placements() {
  ...
  for (size_t i = 0; i < placements_.size(); i++) {
    if (placements_[i] != nullptr) {
      ...
      SPDLOG_INFO("something is wrong, placement is nullptr! i: {}", i);
    }
  }
...
std::vector<std::vector<size_t>> AutoShard::compute_new_alloc_shard(
  ...
  for (size_t i = 0; i < new_ps_shard.size(); i++) {
    if (i < origin_ps_shard.size()) {
      ...
      SPDLOG_INFO("out of range, i: {}, origin_ps_shard.size(): {}", i, origin_ps_shard.size());
    }
  }

  for (size_t i = 0; i < res.size(); i++) {
    if (res[i].size() > 0) {
      SPDLOG_INFO("new_alloc_shard, field: {}, ps: {}", i, absl::StrJoin(res[i], ","));
    }
  }
...
void AutoShard::save_new_shard(const std::string& dirname, const std::string& model_name) {
  ...
  if (ifs.is_open()) {
    ...
    if (ofs_backup.is_open()) {
      ...
      ofs_backup.close();
      SPDLOG_INFO("find auto_shard json, save to backup_filename: {}", backup_filename);
    }
    ifs.close();
  }
  ...
  if (ofs.is_open()) {
    ...
    SPDLOG_INFO("save auto_shard to file: {}", filename);
  }
