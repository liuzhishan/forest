...
`syslibs_configure` depends on the following environment variables:
  ...
  * `KLEARN_SYSTEM_LIBS`: list of third party dependencies that should use
    the system version instead
"""

_KLEARN_SYSTEM_LIBS = "KLEARN_SYSTEM_LIBS"
...
def _enable_syslibs(repository_ctx):
    s = repository_ctx.os.environ.get(_KLEARN_SYSTEM_LIBS, "").strip()
...
def _get_system_lib_list(repository_ctx):
    ...
    """
    if _KLEARN_SYSTEM_LIBS not in repository_ctx.os.environ:
        return []

    libenv = repository_ctx.os.environ[_KLEARN_SYSTEM_LIBS].strip()
    libs = []
