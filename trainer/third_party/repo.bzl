...
def _use_system_lib(ctx, name):
    syslibenv = _get_env_var(ctx, "TRAINER_SYSTEM_LIBS")
    if syslibenv:
...
trainer_http_archive = repository_rule(
    implementation = _trainer_http_archive,
    attrs = {
...
def _third_party_http_archive(ctx):
    ...
    if use_syslib:
        if ctx.attr.system_build_file == None:
            fail("Bazel was configured with TRAINER_SYSTEM_LIBS to use a system " +
...
# Downloads and creates Bazel repos for dependencies.
#
# This is an upgrade for trainer_http_archive that works with go/tfbr-thirdparty.
#
