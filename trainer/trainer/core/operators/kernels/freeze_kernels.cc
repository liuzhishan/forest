...
class FreezeOp : public OpKernel {
 public:
  explicit FreezeOp(OpKernelConstruction* context) : OpKernel(context) {
    ...
    OP_REQUIRES_OK(context, context->GetAttr("dense_var_queues", &dense_var_queues_));
    OP_REQUIRES_OK(context, context->GetAttr("sparse_vars", &sparse_vars_));
    OP_REQUIRES_OK(context, context->GetAttr("sparse_var_queues", &sparse_var_queues_));
  }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(context, hub_eps.size() > 0, errors::InvalidArgument("hub eps empty"));
    ...
    OP_REQUIRES(context, emb_table_nums == sparse_vars_.size(),
    ...
    if (train_config_->embedding_table_store_type().size() == 0) {
      ...
      option.set_embedding_table_store_type(
          train_config_->embedding_table_store_type());
    }
    ...
    *(option.mutable_dense_var_queues()) = {dense_var_queues_.begin(), dense_var_queues_.end()};
    *(option.mutable_sparse_vars()) = {sparse_vars_.begin(), sparse_vars_.end()};
    *(option.mutable_sparse_var_queues()) = {sparse_var_queues_.begin(), sparse_var_queues_.end()};
