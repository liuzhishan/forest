...
void TrainConfig::ConvertConfigToPb(TrainConfig* config,
  ...
  *(option.mutable_feature_list()->mutable_sparse_class_names()) = {
  ...
  for (size_t i = 0; i < config->input_sparse().size(); i++) {
    ...
    auto add_func = [&]() -> void {
      if (config->emb_tables().find(emb_table_name) ==
        ...
        LOG(ERROR) << "emb table not find, name: " << emb_table_name;
        return;
      }
  ...
  } else if (option.src() == sniper::SRC_BTQ) {
    option.mutable_kafka_src()->set_topic(config->topic());
  } else if (option.src() == sniper::SRC_DRAGON) {
    option.mutable_hdfs_src()->set_train_hash(config->train_hash());
  } else {
    LOG(ERROR) << "wrong src: " << option.src();
    return;
  }
...
bool TrainConfig::parse() {
  ...
  LOG(INFO) << "start parse train config!";

  if (!parse_basic(value)) {
    LOG(ERROR) << "parse basic fail";
    return false;
  }
  LOG(INFO) << "start parse train config! parse basic done";
  if (!parse_clusterspec(value)) {
    LOG(ERROR) << "parse clusterspec fail";
    return false;
  }
  LOG(INFO) << "start parse train config! parse cluster done";
  if (!parse_network(value)) {
    LOG(ERROR) << "parse network fail";
    return false;
  }
  LOG(INFO) << "start parse train config! parse network done";
  if (!parse_feature_column(value)) {
    LOG(ERROR) << "parse feature_column fail";
    return false;
  }
  if (!parse_embedding_table(value)) {
    LOG(ERROR) << "parse embedding_table fail";
    return false;
  }
  LOG(INFO) << "start parse train config! parse embedding table done";
  if (!parse_sample(value)) {
    LOG(ERROR) << "parse sample fail";
    return false;
  }
  ...
  LOG(INFO) << "start parse train config! parse trainer done";

  if (!post_parse()) {
    LOG(ERROR) << "post parse check fail";
    return false;
  }

  LOG(INFO) << "start parse train config! post parse done";
  return true;
}
    ...
    if (mem.isMember("shard") && mem["shard"].type() == Json::arrayValue &&
      ...
      for (int i = 0; i < int(mem["shard"].size()); ++i) {
        ...
        *ptr = mem["shard"][i].asString();
        ...
        LOG(INFO) << "shard ps, varname: " << table_name
                  << " " << mem["shard"][i].asString()
                  << ", i: " << i;
        ...
        if (absl::optional<int> int_value = find_int_suffix(table_name)) {
          ...
          if (it_ps_index != ps_to_index_.end()) {
            ...
            LOG(INFO) << "cannot find ps_index: " << mem["shard"][i].asString();
          }
        }
      }

      ps_shard_[table_name] = tmp;
      LOG(INFO) << "ps_shard, key: " << table_name << ", value: " << s;
    } else {
      LOG(INFO) << "sparse feature has no shard, exit! table_name: " << table_name;
      return false;
    }
...
bool TrainConfig::parse_trainer(Json::Value const& value) {
  ...
  LOG(INFO) << "eps: " << eps_
            ...
            << ", l2: " << l2_
            << ", version: " << version_
            << ", use_freq_scale: " << use_freq_scale_;
...
bool TrainConfig::post_parse() {
  ...
  for (auto& dense_feature : input_dense_) {
    if (dense_fcs_.find(dense_feature) == dense_fcs_.end()) {
      LOG(ERROR) << "dense feature not find config, dense feature name: "
                 << dense_feature;
      return false;
    }
  ...
  for (size_t i = 0; i < input_sparse_.size(); i++) {
    ...
    if (sparse_iter == sparse_fcs_.end() && seq_iter == seq_fcs_.end()) {
      LOG(ERROR) << "sparse feature not find config, sparse_feature: "
                 << sparse_feature;
      return false;
    }
    if (sparse_iter != sparse_fcs_.end()) {
      ...
      for (auto& column : columns) {
        ...
        if (emb_tables_.find(emb_table) == emb_tables_.end()) {
          LOG(ERROR) << "emb table not find, emb_table: " << emb_table;
          return false;
        }
  ...
  LOG(INFO) << "Trainer Train Config summary, trainer_id: " << trainer_id_
            << ", batch_size: " << batch_size_
            << ", dense_total_size: " << dense_total_size_
            << ", sparse_total_size: " << sparse_total_size_
            << ", dense_feature_count: " << input_dense_.size()
            << ", sparse_feature_count: " << input_sparse_.size()
            << ", hub_eps_size: " << hub_eps_.size()
            << ", ps_eps_size: " << ps_eps_.size();
...
bool TrainConfig::read_file(const char* path, Json::Value* root) {
  ...
  if (!reader.parse(ifs, *root)) {
    LOG(ERROR) << "parse config failed: path is: " << path;
    return false;
  }
  return true;
}
