...
class RestoreFeatureCountOp : public OpKernel {
 public:
  explicit RestoreFeatureCountOp(OpKernelConstruction* context)
      : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    while (!restore_finish && !restore_error && (--total_iteration > 0)) {
      for (size_t idx = 0; idx < eps.size(); idx++) {
        ...
        auto h = rpc_client_->RestoreFeatureCountAsync(ep, varname_, option,
                                                       &response);
        if (!h->Wait()) {
          LOG(INFO) << "failed" << ", varname: " << varname_;
          LOG(WARNING) << "restore feature count var[" << varname_ << "] on ps["
          ...
          if (!ret_option.errmsg().empty()) {
            LOG(WARNING) << "Trainer restore feature count fail, trainer_id: "
                         << trainer_id_ << ", ep: " << ep
          ...
          OP_REQUIRES(context, restore_error == false,
                      errors::InvalidArgument(
                          std::string("restore feature count fail: ") +
                          ret_option.errmsg()));
        }
    ...
    OP_REQUIRES(context, restore_finish == true,
                errors::InvalidArgument("restore feature count timeout, ",
                                        "var: " + varname_));
    ...
    LOG(INFO) << "restore feature count kernel success, var: " << varname_
              << ", paths: " << absl::StrJoin(paths_, ",") << ", cost: "
...
REGISTER_KERNEL_BUILDER(Name("RestoreFeatureCount").Device(DEVICE_CPU),
                        RestoreFeatureCountOp);
