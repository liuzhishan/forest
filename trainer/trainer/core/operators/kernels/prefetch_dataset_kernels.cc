...
class InputPrefetchDatasetOp : public DatasetOpKernel {
 public:
  explicit InputPrefetchDatasetOp(tensorflow::OpKernelConstruction* ctx) : DatasetOpKernel(ctx) {
  ...
  void MakeDataset(OpKernelContext* ctx, DatasetBase** output) override {
    OP_REQUIRES_OK(
        ctx, tensorflow::data::ParseScalarArgument<std::string>(ctx, "conf_file", &conf_file_));
    OP_REQUIRES_OK(ctx, tensorflow::data::ParseScalarArgument<tensorflow::int32>(ctx, "trainer_id",
                                                                                 &trainer_id_));
    OP_REQUIRES_OK(ctx, tensorflow::data::ParseScalarArgument<tensorflow::int32>(ctx, "work_mode",
                                                                                 &work_mode_));

    *output = new Dataset(ctx, conf_file_, trainer_id_, output_types_, output_shapes_, work_mode_);
  }

 private:
  class Dataset : public DatasetBase {
   public:
    Dataset(OpKernelContext* ctx, const std::string& conf_file, int32_t trainer_id,
            const DataTypeVector& output_types,
            const std::vector<tensorflow::PartialTensorShape>& output_shapes, int32_t work_mode)
    ...
    std::string DebugString() const override { return "InputPrefetchDatasetOp::Dataset"; }

   protected:
    tensorflow::Status AsGraphDefInternal(SerializationContext* ctx, DatasetGraphDefBuilder* b,
      ...
      TF_RETURN_IF_ERROR(b->AddDataset(this, {conf_file, trainer_id, work_mode}, output));
   ...
   private:
    class Iterator : public DatasetIterator<Dataset> {
     public:
      ...
      tensorflow::Status GetNextInternal(IteratorContext* ctx, std::vector<Tensor>* out_tensors,
        ...
        for (size_t i = 0; i < input_sparse.size(); ++i) {
          ...
          for (int j = 0; j < size; ++j) {
            if (std::isnan(flat(j))) {
              LOG(FATAL) << "embedding lookup ret has nan, i: " << i << ", j: " << j
                         << ", flat.size: " << flat.size();
            }
          }
...
REGISTER_KERNEL_BUILDER(Name("InputPrefetchDataset").Device(DEVICE_CPU), InputPrefetchDatasetOp);
