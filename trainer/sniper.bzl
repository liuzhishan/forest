...
def sniper_go_grpc_library(**kwargs):  # pylint: disable=unused-argument
    """Build the Go gRPC bindings for a service. Not yet implemented."""
    return

def sniper_proto_library_py(name, proto_library, srcs = [], deps = [], visibility = None, testonly = 0):  # pylint: disable=unused-argument
    py_proto_library(
        name = name,
        srcs = srcs,
...
def sniper_tensorflow_proto_dep(dep):
    """Rename for deps onto tensorflow protos in sniper_proto_library targets.
    """
